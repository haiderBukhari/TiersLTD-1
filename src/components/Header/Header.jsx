    ...
    return (
        <div className="flex flex-col md:flex-row gap-5 items-start justify-between px-10 py-3 font-semibold text-white leading-[100%] flex-wrap w-full" style={{ zIndex: 20 }}>
            ...
            <div className={`flex flex-col md:flex-row justify-between mt-10 md:mt-4 md:h-auto gap-5 pr-5 my-auto text-xs font-light ${show ? 'block' : 'hidden md:block'}`} style={{ fontFamily: "Krona One" }}>
                <Link to="/miner" onClick={() => { setShow(!show) }} className="cursor-pointer mt-8 md:mt-0 mx-0 md:mx-4" style={{ zIndex: 20 }} >ASICS</Link>
                <Link to="/miner2" onClick={() => { setShow(!show) }} className="cursor-pointer mt-8 md:mt-0 mx-0 md:mx-4" style={{ zIndex: 20 }} >Unser Angebot</Link>
                <Link to="/miner3" onClick={() => { setShow(!show) }} className="cursor-pointer mt-8 md:mt-0 mx-0 md:mx-4" style={{ zIndex: 20 }} >Pakete</Link>
                <Link to="/miner4" onClick={() => { setShow(!show) }} className="cursor-pointer mt-8 md:mt-0 mx-0 md:mx-4" style={{ zIndex: 20 }} >Kontakt</Link>
            </div>
