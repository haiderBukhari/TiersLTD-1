...
export function Header({ show, setShow }) {
    return (
        <div className="flex flex-col md:flex-row gap-5 items-start justify-between px-10 py-3 font-semibold text-white leading-[100%] flex-wrap w-full" style={{ zIndex: 20 }}>
            ...
            <div className={`flex flex-col md:flex-row w-full md:w-auto justify-center md:justify-between mt-10 md:mt-4 md:h-auto gap-5 pr-5 my-auto text-xs font-light ${show ? 'block' : 'hidden md:block'}`} style={{ fontFamily: "Krona One" }}>
            ...
            <Link to="/register" style={{ zIndex: 20 }} className={`justify-center px-3 py-3.5 text-base text-center whitespace-nowrap rounded-xl border border-solid bg-[linear-gradient(225deg,#18C8FF_14.89%,#933FFE_85.85%)] border-white border-opacity-50 mt-20 md:mt-0 ${show ? 'block mx-auto' : 'hidden md:block'}`}>
                Kundenportal
            </Link>
        </div>
    );
